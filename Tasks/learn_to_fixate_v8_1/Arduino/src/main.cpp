...
void move_fix_dur(float percent_change){
    fix_dur += fix_dur * percent_change;
    fix_dur = constrain(fix_dur, fix_dur_start, fix_dur_target);
    log_float("fix_dur",fix_dur);
}
    ...
    switch (current_state) {
        ...
        case FIXATE_STATE:
            ...
            if (current_state != last_state){
                ...
                log_code(FIXATE_ON);
                lights_on();
                this_fix_dur = random(fix_dur - fix_window, fix_dur + fix_window);

            // exit conditions
            if (in_fix_box == false){
                // trial aborted - premature choice
                log_code(TRIAL_ABORTED_EVENT);
                log_code(FIXATION_BROKEN_EVENT);
                log_code(FIXATE_OFF);

                // decrease fix duration
                move_fix_dur(fix_dur_decrement);
            ...
            if (now() - t_state_entry > this_fix_dur){
                // successfully fixated
                log_code(FIXATE_OFF);
                log_code(FIXATION_SUCCESSFUL_EVENT);

                // increase fix duration
                move_fix_dur(fix_dur_increment);
