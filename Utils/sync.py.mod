...
class Syncer(object):
    ...
    def fit(self, data_A, data_B, order=2):
        """ linreg sync of A to B """
        ...
        p0 = self.guess_p0(fitfun, data_A, data_B)
        pfit = curve_fit(fitfun, data_A, data_B, p0=p0)[0]
    ...
    def sync(self, A, B, check=True, symmetric=True, order=2, pieces=1):
        """ linreg sync of A to B """
        # check and abort if fails
        # this can be made mandatory actually
        if check:
            success = self.check(A, B)
            if not success:
        ...
        splits_A, splits_B = self.make_splits_for_piecewise(A,B,N=pieces)
        pfits = []
        fitfuncs = []
        for i in range(pieces):
            pfit, fitfun = self.fit(splits_A[i], splits_B[i], order=order)
            pfits.append(pfit)
            fitfuncs.append(fitfun)

        self.pairs[(A,B)] = pfits
        self.funcs[(A,B)] = fitfuncs
        if A in self.graph:
            self.graph[A].append(B)
        else:
            self.graph[A] = [B]
    ...
    def _convert(self, t, A ,B):
        ...
        for i in range(pieces):
            t_min = splits_A[i][0]
            t_max = splits_A[i][-1]
            binds = np.logical_and(t >= t_min, t <= t_max)
            yhat.append(yhats[i][binds])
        yhat = np.concatenate(yhat)
