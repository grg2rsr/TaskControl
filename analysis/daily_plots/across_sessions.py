...
def plot_anticipatory_reaches_over_sessions(Animal_folder, save=None):
    ...
    for i, row in SessionsDf.iterrows():
        ...
        try:
            ...
            try:
                SessionsDf.loc[i, "f_anticip_left"] = (
                    groupby_dict(
                        SessionDf,
                        dict(
                            has_choice=True, has_anticip_reach=True, chosen_side="left"
                        ),
                    ).shape[0]
                    / n_trials
                )
                ...
                pass
            try:
                SessionsDf.loc[i, "f_anticip_right"] = (
                    groupby_dict(
                        SessionDf,
                        dict(
                            has_choice=True, has_anticip_reach=True, chosen_side="right"
                        ),
                    ).shape[0]
                    / n_trials
                )
    ...
    axes.plot(
        tvec,
        SessionsDf["f_anticip_left"].values + SessionsDf["f_anticip_right"].values,
        color=colors["both"],
    )
...
for i, row in SessionsDf.iterrows():
    ...
    metrics = (
        ...
        m.get_correct_side,
        m.get_chosen_side,
        m.has_premature_reach,
        m.has_premature_choice,
    )
    SessionDf, TrialDfs = bhv.get_SessionDf(LogDf, metrics)
    ...
    try:
        ...
        try:
            SessionsDf.loc[i, "f_prem_reach_left"] = (
                groupby_dict(
                    SessionDf,
                    dict(has_choice=True, has_premature_reach=True, chosen_side="left"),
                ).shape[0]
                / n_trials
            )
        except KeyError:
            ...
            pass
        try:
            SessionsDf.loc[i, "f_prem_reach_right"] = (
                groupby_dict(
                    SessionDf,
                    dict(
                        has_choice=True, has_premature_reach=True, chosen_side="right"
                    ),
                ).shape[0]
                / n_trials
            )
    ...
    try:
        ...
        try:
            SessionsDf.loc[i, "f_prem_choice_left"] = (
                groupby_dict(
                    SessionDf,
                    dict(
                        has_choice=True, has_premature_choice=True, chosen_side="left"
                    ),
                ).shape[0]
                / n_trials
            )
            ...
            pass
        try:
            SessionsDf.loc[i, "f_prem_choice_right"] = (
                groupby_dict(
                    SessionDf,
                    dict(
                        has_choice=True, has_premature_choice=True, chosen_side="right"
                    ),
                ).shape[0]
                / n_trials
            )
...
colors = dict(left=mpl.cm.PiYG(0.05), right=mpl.cm.PiYG(0.95), both="black")
...
axes[0].plot(
    tvec,
    SessionsDf["f_prem_reach_left"].values + SessionsDf["f_prem_reach_right"].values,
    color=colors["both"],
)
...
axes[1].plot(
    tvec,
    SessionsDf["f_prem_choice_left"].values + SessionsDf["f_prem_choice_right"].values,
    color=colors["both"],
)
