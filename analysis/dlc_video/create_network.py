...
video_paths = ["/media/georg/htcondor/shared-paton/georg/Animals_reaching/JJP-02994_Plumber/2021-10-19_12-10-36_learn_to_choose_v2/bonsai_video.avi",
...
net_type = "resnet_50"
date = datetime.now().strftime("%Y-%m-%d") # underscores in times bc colons kill windows paths ...
bodyparts = "paws-spouts-tongue"
training_data_dir = working_dir / ('_'.join([date, bodyparts, net_type, 'training_videos']))
...
# %%
config_path = dlc.create_new_project('_'.join([date, bodyparts, net_type]), 'georg', training_video_paths)
...
dlc.create_training_dataset(config_path, net_type="mobilenet_v2_0.75", augmenter_type='imgaug')
dlc.train_network(config_path)

# %% evaluate training 
import pandas as pd
import matplotlib.pyplot as plt
Df = pd.read_csv("/media/georg/data/reaching_dlc/paws_only-georg-2021-06-02/dlc-models/iteration-0/paws_onlyJun2-trainset95shuffle1/train/learning_stats.csv",names=['it','loss','lr'])
...
dlc.analyze_videos(config_path, vid_paths) # seems like it crashed with dynamic, dynamic=(True, 0.5, 10))
