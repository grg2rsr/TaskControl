...
session_folder = Path("/media/georg/htcondor/shared-paton/georg/Animals_smelling_imaging/JJP-05425/2023-02-08_11-39-35_twodistributionsv6_GR")
...
for i,delay in enumerate(delays):
    ...
    for j, row in Df.iterrows():
        ...
        ix = np.logical_and(lick_rate_tvec*1e3 > t0 + pre, lick_rate_tvec*1e3 < t0 + post)
...
fig, axes = plt.subplots(nrows=n_delays, gridspec_kw=dict(height_ratios=n_trials_per_delay), sharex=True)
twin_axes = [plt.twinx(ax) for ax in axes]

for i,delay in enumerate(delays):
    for j, t_licks in enumerate(lick_times[delay]):
        y = np.ones(t_licks.shape[0]) * j
        axes[i].plot(t_licks, y, '.', color='k', alpha=0.5, markeredgewidth=0, markersize=3.5)
    ...
    twin_axes[i].fill_between(tvec, lick_rate_lower, lick_rate_upper, linewidth=0, alpha=0.4, color=delay_colors[i])

max_rate = np.max([np.max(np.average(lick_rates_delay[delay],axis=0)) for delay in delays])
